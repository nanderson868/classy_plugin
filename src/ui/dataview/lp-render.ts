		...
		class {
			...
			addDeco(node: SyntaxNode, view: EditorView) {
				...
				if (!exists) {
					...
					const newDeco = this.renderWidget(
						node,
						view,
						currentFile
					)?.value;
			...
			isInlineQuery(view: EditorView, start: number, end: number) {
				...
				const isInlineQuery = false; // text.startsWith(settings.inlineQueryPrefix) || text.startsWith(settings.inlineJsQueryPrefix);
			...
			renderWidget(
				...
				// 		code = text
				// 			.substring(settings.inlineQueryPrefix.length)
				// 			.trim();
				...
				// 		code = text
				// 			.substring(settings.inlineJsQueryPrefix.length)
				// 			.trim();
				// 		try {
