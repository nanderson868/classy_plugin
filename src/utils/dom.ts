...
// 	if (!node) {
// 		log.error("element with data path not found", path);
// 		return;
// 	}
...
// 	} else {
// 		const parsedEmoji =
// 			emoji.parseEmoji(plugin.getSettings().emojiStyle, iconName) ??
// 			iconName;
...
const createIconNode = (
	...
	if (!titleNode) {
		...
		if (!titleNode) {
			log.error("element with title not found");
			return;
		}
	}

	let iconNode: HTMLDivElement | null = node.querySelector(".iconize-icon");
