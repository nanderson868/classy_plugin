...
const log = new Logger().setContext("IconPackManager");
...
const createDirectory = async (
	...
	const doesDirExist = await plugin.app.vault.adapter.exists(
		`${path}/${dir}`
	);
	if (!doesDirExist) {
...
const generateIcon = (
	...
	if (!validIconName.exec(normalizedName)) {
		log.debug({}, `skipping icon with invalid name: ${iconName}`);
		return null;
	}
	...
	if (!svgContentMatch) {
		log.debug({}, `skipping icon with invalid svg content: ${content}`);
		return null;
	}
...
export const loadUsedIcons = async (plugin: MyPlugin, icons: string[]) => {
	const iconPacks: string[] = (await listPath(plugin)).folders
		.map((iconPack) => iconPack.split("/").pop() ?? "")
		.filter(Boolean);
...
export const getIconPackNameByPrefix = (prefix: string): string => {
	const iconPack = iconPacks.find((iconPack) => iconPack.prefix === prefix);
...
export const loadIcon = async (
	...
	if (icon) preloadedIcons.push(icon);
};
...
export const initIconPacks = async (plugin: Plugin): Promise<void> => {
	...
	// 		let iconPackName = fileName.split("/").pop();
	// 		if (!iconPackName) return;
	// 		iconPackName = iconPackName.split(".zip")[0];
	...
	// 	if (folderName && zipFiles[folderName]) {
	// 		continue;
	// 	}
	...
	// 		);
	// 		if (!iconNameRegex || !folderName) return;
	...
	// 	log.debug({}, folderName, loadedIcons);
	// 	iconPacks.push({
	// 		name: folderName,
	// 		icons: loadedIcons,
	// 		prefix,
	// 		custom: true,
	// 	});
	// 	log.debug({}, `loaded icon pack ${folderName} (${loadedIcons.length})`);
	// }
	// Extract all files from the zip files.
	// for (const zipFile in zipFiles) {
	// 	const files = zipFiles[zipFile];
	// 	const loadedIcons: Icon[] = await getLoadedIconsFromZipFile(
	// 		zipFile,
	// 		files
	// 	);
	...
	// 		custom: false,
	// 	});
	// 	log.debug({}, `loaded icon pack ${zipFile} (${loadedIcons.length})`);
	// }
};
...
// export const removeIconFromIconPackDirectory = async (
...
// 	log.debug({}, `loaded icon pack ${name} (${loadedIcons.length})`);
// };
